...
let rec fuel_of_fuel mass =
  ...
  if rem > 0 
  then fuel_of_fuel rem + rem
  else 0

let lines = Utils.input_lines input_file

let () =  
  let sum = List.fold_right (fun x y -> (fuel @@ int_of_string x) + y) lines 0 in
  print_endline @@ string_of_int sum

let () =
  let sum = List.fold_right (fun x y -> (fuel_of_fuel @@ int_of_string x) + y) lines 0 in
  print_endline @@ string_of_int sum
