...
let parse_cmd state suffix n_args =
  ...
  | false ->
      Error
        (Printf.sprintf "Parser error: Could not parse the next input: %s\n"
           token)
...
let parse_change_rb state =
  Result.map (parse_op_1 state 9) ~f:(fun x -> ChangeRB x)
